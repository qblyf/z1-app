...
/// 库存盘点列表页
class StocktakingListPage extends ConsumerStatefulWidget {
  const StocktakingListPage({super.key});

  @override
  ConsumerState<StocktakingListPage> createState() => _StocktakingListPageState();
}

class _StocktakingListPageState extends ConsumerState<StocktakingListPage> {
  List<StocktakingLog> _list = [];
  ...
  void _showFilterSheet() {
    showCupertinoModalPopup(
      context: context,
      builder: (_) => _StocktakingFilterSheet(
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('暂无盘点记录', style: AppText.caption),
                        ],
                      ),
                    )
                          ...
                          SliverPadding(
                            ...
                            sliver: SliverList(
                              delegate: SliverChildBuilderDelegate(
                                (context, i) => _StocktakingCard(
                                  log: _list[i],
                                  onTap: () => context.push('/stocktaking/take/${_list[i].stocktakingLogID}'),
                                ),
...
class _StocktakingCard extends StatelessWidget {
  final StocktakingLog log;
  final VoidCallback onTap;

  const _StocktakingCard({required this.log, required this.onTap});
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Text(log.type.label, style: const TextStyle(fontSize: 11, color: Color(0xFF5E5CE6))),
                ),
                const SizedBox(width: 8),
                Text('${log.creatorName ?? '-'}', style: AppText.caption),
              ],
            ),
...
class _StocktakingFilterSheet extends StatefulWidget {
  final int? statusFilter;
  final DateTime startDate;
  final DateTime endDate;
  final void Function(int?, DateTime, DateTime) onApply;

  const _StocktakingFilterSheet({
  ...
  @override
  State<_StocktakingFilterSheet> createState() => _StocktakingFilterSheetState();
}

class _StocktakingFilterSheetState extends State<_StocktakingFilterSheet> {
  ...
  void initState() {
    ...
    _statusIndex = widget.statusFilter == null
        ? 0
        : StocktakingState.values.indexWhere((s) => s.value == widget.statusFilter) + 1;
  ...
  int? get _selectedStatus {
    ...
    final allStatuses = [null, ...StocktakingState.values];
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              ...StocktakingState.values.asMap().entries.map((e) =>
                _StatusChip(
                  label: e.value.label,
